...
export const top_level_agent_schema = z.object({
    ...
    updatedInstruction: z
        .string()
        .describe(
            'updated instruction with combining past history',
        ),
});
...
export const executor_schema = z.object({
    ...
    questions: z.array(question_schema).min(1).max(10),
...
export const planner_schema = z.object({
    userResponse: z
        ...
        .max(200)
        .describe(
            'give a user response like you are doing the job act normal, no need of much appraisal',
        ),
    ...
    operationType: z.enum(['REPLACE', 'APPEND']).describe('The classified intent of the user message'),
});
