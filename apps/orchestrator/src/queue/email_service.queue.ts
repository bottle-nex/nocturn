...
export default class EmailServiceProcessor {
    ...
    this.email_queue.process(async (job: Job<EmailJob>) => {
      ...
      switch (type) {
        case EmailJobType.COLLABORATOR_ADDED:
          return this.handle_collaborator_added_email(
            data as CollaboratorAddedEmailData,
          );
        case EmailJobType.COLLABORATOR_INVITE:
          return this.handle_collaborator_invite_email(
            data as CollaboratorInviteEmailData,
          );
        default:
  ...
  private async handle_collaborator_added_email(
    data: CollaboratorAddedEmailData,
  ) {
    console.log("Handling collaborator added email", data);
  }

  private async handle_collaborator_invite_email(
