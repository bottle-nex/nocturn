...
import {
    Users, CalendarCheck, Layers, BarChart3,
    Palette, Users2, Edit3, MonitorPlay, Star,
    LucideIcon
} from "lucide-react";
                ...
                ${isDark ? "border-r border-white/7" : "border-r border-black/7"}`}
                            >
                                <div className="flex flex-col">
                                    <span className={`text-[22px] font-bold tracking-tight leading-none ${isDark ? "text-[#f0ede8]" : "text-[#1a1a1a]"}`}>
                                        {tier.name}
                                    </span>
                                    <span className={`text-[13px] mt-0.5 ${isDark ? "text-[#f0ede8]/45" : "text-[#1a1a1a]"}`}>
                                        membership
                                    </span>
                                ...
                                <div>
                                    <div className={`flex items-end gap-1 ${isDark ? "text-[#f0ede8]" : "text-[#1a1a1a]"}`}>
                                        <span className="text-[32px] font-bold tracking-tight leading-none">
                                            {tier.price.currency === "EUR" ? "€" : "$"}{tier.price.amount}
                                        </span>
                                        <span className={`text-xs pb-1 ${isDark ? "text-[#f0ede8]/60" : ""}`}>
                                            {priceLabel}
                                        </span>
                                    </div>
                                    {billedLabel && (
                                        <div className={`text-[11px] mt-1 ${isDark ? "text-[#f0ede8]/40" : "text-[#1a1a1a]/45"}`}>
                                            {billedLabel}
                                        </div>
                                    )}
                                    <Button
                    ...
                    ${isDark
                        ...
                        ${isDark ? "bg-white/8" : "bg-black/6"}`}
                                            ...
                                            <div>
                                                <div className={`text-[11.5px] font-medium leading-tight ${isDark ? "text-[#e8e4dc]" : "text-[#1a1a1a]"}`}>
                                                    {feature.label}
                                                </div>
