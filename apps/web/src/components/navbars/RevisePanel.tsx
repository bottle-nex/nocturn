...
export default function RevisePanel({ onBackgroundClick, reviseQuizPanel, onConfirm, isLoading }: RevisePanelProps) {
    ...
    const stepsContent = [
        {
            ...
            content: (
                <div className="flex flex-col gap-y-4 text-neutral-800 dark:text-neutral-200 mt-2">
                    <p className="text-sm opacity-70 mb-2">Check your selected theme and total questions.</p>
            ...
            content: (
                <div className="flex flex-col gap-y-3 text-neutral-800 dark:text-neutral-200 mt-2">
                    ...
                    <div className="flex justify-between items-center bg-white/5 border border-neutral-200 dark:border-neutral-800 p-4 rounded-xl shadow-sm mt-1">
                        ...
                        <span className="font-semibold text-lg px-2 py-0.5 bg-indigo-600/10 rounded-md text-indigo-600 dark:text-indigo-400">{quiz.pointsMultiplier || 1}x</span>
                    </div>
                </div>
            )
        },
        {
            ...
            content: (
                <div className="flex flex-col gap-y-4 text-neutral-800 dark:text-neutral-200 mt-2">
                    <p className="text-sm opacity-70 mb-2">Reactions and interactive modules selected.</p>
                    {quiz.interactions.length > 0 ? (
                        <div className="flex flex-wrap gap-2 p-2">
                            {quiz.interactions.map(interaction => (
                                <span key={interaction} className="px-4 py-2 border border-neutral-300 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-800/80 rounded-xl text-sm font-medium shadow-sm">
                                    {interaction}
                                </span>
                            ))}
                        </div>
                        ...
                        <div className="bg-neutral-100/50 dark:bg-neutral-800/30 border border-neutral-200 dark:border-neutral-800 p-8 rounded-xl text-center flex flex-col items-center justify-center h-[120px]">
                            <span className="opacity-50 text-sm font-medium">No interactions enabled</span>
                        </div>
                    )}
                </div>
            )
        },
        {
            ...
            content: (
                <div className="flex flex-col gap-y-4 text-neutral-800 dark:text-neutral-200 mt-2">
                    <p className="text-sm opacity-70 italic mb-2">These settings can also be toggled during the live quiz.</p>
                    <div className="flex justify-between items-center bg-white/5 border border-neutral-200 dark:border-neutral-800 p-4 rounded-xl shadow-sm">
                        <span className="font-medium">Live Chat:</span>
                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${quiz.liveChat ? 'bg-green-500/20 text-green-600 dark:text-green-500 border border-green-500/30' : 'bg-red-500/10 text-red-600 dark:text-red-500 border border-red-500/20'}`}>
                            {quiz.liveChat ? 'Enabled' : 'Disabled'}
                        </span>
                    </div>
                    <div className="flex justify-between items-center bg-white/5 border border-neutral-200 dark:border-neutral-800 p-4 rounded-xl shadow-sm">
                        <span className="font-medium">Spectator Mode:</span>
                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${quiz.spectatorMode ? 'bg-green-500/20 text-green-600 dark:text-green-500 border border-green-500/30' : 'bg-red-500/10 text-red-600 dark:text-red-500 border border-red-500/20'}`}>
                    ...
                    {quiz.prizePool > 0 ? (
                        <>
                            <div className="flex justify-between items-center bg-gradient-to-r from-green-500/5 to-emerald-500/10 border border-green-500/20 p-4 rounded-xl shadow-sm">
                                ...
                                <span className="font-bold text-green-600 dark:text-green-500 text-xl tracking-tight">{quiz.prizePool} {quiz.currency || 'USDC'}</span>
                            </div>
                            <div className="flex justify-between items-center bg-white/5 border border-neutral-200 dark:border-neutral-800 p-4 rounded-xl shadow-sm">
                                <span className="font-medium">Total Winners:</span>
                                <span className="font-semibold px-3 py-1 bg-neutral-200 dark:bg-neutral-800 rounded-full">{quiz.prizeDistributions?.length || 0}</span>
                            </div>
                            <div className="p-4 border border-dashed border-neutral-400 dark:border-neutral-700 rounded-xl flex flex-col gap-y-3 relative mt-2 bg-neutral-50 dark:bg-neutral-900/40">
                                <span className="text-xs font-semibold opacity-60 uppercase tracking-widest text-[10px]">Staked From Address</span>
                                <div className="flex items-center justify-between">
                                    ...
                                    <Button size="sm" variant="outline" className="h-[28px] text-xs px-3 dark:border-neutral-700 bg-white dark:hover:bg-neutral-800 shadow-sm rounded-lg">Set Address</Button>
                                </div>
                            </div>
                        </>
                    ) : (
                        <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-neutral-300 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/20 rounded-xl gap-y-4 my-2">
                            <span className="text-center text-sm font-medium opacity-80 max-w-[250px]">No staked prize found for this quiz.</span>
                            <Button size="sm" onClick={() => toast.success('Configure prize in the settings panel')} className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-md shadow-indigo-600/20 px-6">
                                Add Prize
                            </Button>
                        </div>
                    )}
