...
export default function Navbar() {
    ...
    return (
        <div className="w-full h-20 px-3 fixed z-40 flex items-center justify-between ">
            <div className="h-15 w-15 rounded-full text-gamma bg-black flex justify-center items-center ">
                <TbAlphabetHebrew size={'25'} />
            </div>

            <div className="flex">
                <NavItems items={navItems} className="absolute left-1/2 top-2 -translate-x-1/2 " />
            </div>

            <div className="flex h-15 w-fit rounded-4xl p-1 border-2 border-black bg-gamma ">
                <div className="h-full rounded-full flex items-center justify-center ">
                    <div
                        className={cn(
                            'h-13 w-13 relative rounded-full border-2 border-black bg-alpha overflow-hidden flex justify-center items-center text-black ',
                            'cursor-pointer',
                        )}
                    ...
                    >
                        {session?.user.image ? (
                            ...
                            <RiLoginCircleLine size={'20'} className={cn('cursor-pointer')} />
                        )}
                    </div>
