    ...
    return (
        <div className="flex flex-col gap-y-1.5 px-3 mt-3">
            {steps.map((s, i) => (
                <div
                    key={i}
                    className="flex items-center gap-x-2 bg-light-alpha rounded-lg px-2.5 py-1.5 ring-1 ring-black/5 shadow-xs shadow-black/5"
                >
                    <div
                        className={`h-4 w-4 rounded-full flex items-center justify-center shrink-0 ${s.done ? 'bg-dark-base' : 'ring-1 ring-black/20'}`}
                    >
                        {s.done && (
                    ...
                    <span
                        className={`text-[10px] ${s.done ? 'text-dark-base/50' : 'text-dark-base/70'}`}
                    >
                        {s.label}
                    </span>
...
export default function BeginnerGuideCard() {
    return (
        <div className="h-65 w-65 ring-1 ring-black/10 rounded-xl flex flex-col overflow-hidden shadow-sm shadow-black/10 bg-[#f8ffff] absolute left-[26%] rotate-2 top-30 select-none">
