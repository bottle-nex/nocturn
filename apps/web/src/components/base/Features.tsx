        ...
        <div className='w-full bg-[#90a7ed] h-[calc(100vh-160px)] py-12 flex flex-col justify-center'>
            <div className='max-w-7xl mx-auto'>
                <div className='relative mx-auto w-fit flex items-center justify-center'>
                    <div className='text-center text-6xl font-semibold w-full text-black'>Features of Nocturn</div>
            ...
            <section className={cn('grid grid-cols-3 gap-6 max-w-6xl mx-auto mt-12 px-4',
                ...
                {features.map((feature, idx) => (
                    <div className={cn('col-span-1 border-2 border-black bg-white shadow-hard w-full h-48 transition-transform duration-200 ease-out hover:translate-x-0.5 hover:translate-y-0.5 hover:scale-99',
                        "flex flex-col items-start justify-end px-6 py-4"
                    )} key={idx}>
                        <h1 className='text-2xl font-bold text-black'>{feature.title}</h1>
                        <span className='text-base font-normal text-black'>{feature.description}</span>
                    </div>
                ))}
            </section>
        </div>
